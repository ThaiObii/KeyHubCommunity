...
-- Function to verify key with retry logic, SSL bypass, and fallback
local function verifyKey(key, maxRetries, retryDelay)
    print("Starting key verification for key: " .. key)
    local retries = 0
    while retries < maxRetries do
        local success, response
        -- Try RequestInternal with IgnoreSslValidation
        ...
        if not success then
            ...
            -- Fallback to GetAsync if RequestInternal is unsupported
            success, response = pcall(function()
        ...
        if success then
            print("Received response: " .. tostring(response))
            ...
            if decodeSuccess then
                ...
                print("Failed to parse API response: " .. tostring(response))
...
-- Verify key with temporary bypass for testing
print("Verifying key...")
local success, result
local bypassVerification = true -- Set to false for normal operation
if bypassVerification then
    print("Bypassing key verification for testing...")
    success = true
    result = {valid = true, redeemed = true, exists = true} -- Mock valid response
else
    success, result = verifyKey(getgenv().Key, 3, 1)
end

if not success then
    Player:Kick("Lỗi: " .. result .. ". \n- Đảm bảo API đang chạy tại " .. API_URL .. ". \n- Kiểm tra kết nối mạng hoặc dùng URL công khai. \n- Nếu dùng chứng chỉ tự ký, triển khai API với chứng chỉ đáng tin cậy. \nLiên hệ quản trị viên hoặc thử lại sau.")
    return
end
...
print("Key verified successfully. Loading UI...")
...
if not successUI then
    ...
    Player:Kick("Lỗi: Không thể tải giao diện. Liên hệ quản trị viên.")
    return
end
